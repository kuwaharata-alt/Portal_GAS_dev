...
function SV_TotalWork() {
  const startRowSV = 425
  const lastRowSV = getLastRowByColumn(sheetSV, SVCOL_MITSUMORI) 
  if (startRowSV === null ) return;
  Logger.log("〇作業工数追加");
  
  for (let row = startRowSV; row <= lastRowSV; row++) {

    try {
      const sheetName = sheetSV.getRange(row, 3).getDisplayValue(); // C列
      const linkCell  = sheetSV.getRange(row, 4);                   // D列：HYPERLINK
      ...
      // ▼ 元シートの K列（11列）に書き込み
      sheetSV.getRange(row, 11).setValue(uValue);
...
function extractFileId(url) {
  ...
  throw new Error("fileId が抽出できませんでした：" + url);
}
