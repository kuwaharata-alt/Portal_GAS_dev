function api_getEdit(req) {
  try {
    ...
    const values = sh
      .getRange(1, 1, Math.min(lastRow, WEBVIEW.MAX_ROWS || 5000), lastCol)
      .getDisplayValues();
    ...
    if (idxKey === undefined)
      return { ok: false, error: `ヘッダー「${cfg.keyHeader}」が見つかりません` };

    let rowIndex = -1;
    for (let i = 1; i < values.length; i++) {
      if (String(values[i][idxKey] ?? '').trim() === key) {
        rowIndex = i;
        break;
      }
    }
...
function api_updateFields(req) {
  try {
    ...
    if (!targets.length)
      return { ok: false, error: '更新対象がありません（editableDef/ヘッダー存在を確認）' };
